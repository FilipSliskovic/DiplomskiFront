      ...
      <v-col>
        ...
        >
            ...
            <v-toolbar flat>
              ...
              <v-dialog transition="dialog-top-transition" max-width="450">
                <template v-slot:activator="{ props }">
                  <v-btn color="primary" v-bind="props" @click="getTables()"
                    >New Order</v-btn
                  >
                </template>
                  ...
                  <v-card>
                    ...
                    <v-card-actions class="justify-end">
                      <v-btn
                        variant="text"
                        @click="
                          createNewOrder(this.SelectedTable);
                ...
                <v-card>
                  ...
                  <v-card-text>
                    <v-container v-if="SingleOrder.length">
                      <v-card
                        v-for="cafeProductOrder in SingleOrder"
                        :key="cafeProductOrder.id"
                        variant="outlined"
                      >
                        <v-row align="center">
                          <v-col>
                            <v-card-text>
                              Name:
                              {{ cafeProductOrder.cafeProducts.productName }}
                            </v-card-text>
                          </v-col>
                          <v-col>
                            <v-card-text>
                              Price per:
                              {{ cafeProductOrder.cafeProducts.price }}RSD
                            </v-card-text>
                          </v-col>
                          <v-col>
                            <v-card-text>
                          ...
                          <v-col>
                            <v-card-text>
                              Total:
                              {{ cafeProductOrder.totalProductsPrice }}RSD
                            </v-card-text>
                          </v-col>
                          <v-col>
                            <v-btn
                              ...
                              color="error"
                              @click="
                                deleteCafeProductOrder(cafeProductOrder.id)
                              "
                            >
                              Delete
                            </v-btn>
          ...
          <template v-slot:[`item.Actions`]="{ item }">
            <v-icon size="small" class="me-2" @click="getSingleOrder(item.raw)">
              mdi-pencil
            </v-icon>
            <v-icon
              size="small"
...
export default {
  ...
  methods: {
    ...
    getOrders() {
      ...
      axios
        .get(
          "http://localhost:5000/api/orders?workerid=" +
    ...
    getWorkersCafe() {
      ...
      axios
        .get(
          "http://localhost:5000/api/workerscafe?workerid=" +
            this.$store.getters.userId,
          {
            headers: { Authorization: "Bearer " + this.$store.getters.Token },
          }
    ...
    getTables() {
      ...
      if (this.Orders) {
        axios
          .get(
            "http://localhost:5000/api/table?keyword=" +
              this.WorkerCafe[0].cafeName,
            {
              headers: { Authorization: "Bearer " + this.$store.getters.Token },
            }
          )
    ...
    getSingleOrder(order) {
      if (order !== undefined) {
        ...
        axios
          .get(
            "http://localhost:5000/api/cafeproducts?CafeName=" + order.cafeName,
            {
              headers: { Authorization: "Bearer " + this.$store.getters.Token },
            }
          )
      ...
      axios
        .get(
          "http://localhost:5000/api/CafeProductOrders?OrderId=" +
    ...
    save() {
      ...
      axios
        .post(
          "http://localhost:5000/api/CafeProductOrders",
          {
            cafeProductId: this.SelectedCafeProductId,
